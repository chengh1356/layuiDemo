    ...
    function(exports) {
        ...
        self.tab = {
                ...
                refresh: function(url) {
                    url = url || layui.chen.route.fileurl;
                    if (this.has(url)) {
                        this.del(url, true);
                        self.renderTabs(url);
                    }
                ...
                clear: function() {
                    this.data = [];
                    this.isInit = false;
                    $(document).off('click', this.wrap + ' .chen-tabs-btn');
                },
                change: function(route, callback) {
                    ...
                    var changeView = function(lay) {
                        $('#' + conf.containerBody + ' > .chen-tabs-item' + lay)
                            .show()
                            .siblings()
                            .hide()
                    };
                    var lay = '[lay-url="' + fileurl + '"]';
                    var activeCls = 'chen-tabs-active';
                    ...
                    if (existsTab) {
                        ...
                        layui.chen.navigate(route.href);
                    } else {
                        self.loadHtml(fileurl, function(res) {
                            var htmlElem = $(
                                '<div>' +
                                '<div class="chen-tabs-item" lay-url="' + fileurl + '">' +
                                res.html +
                                '</div>' +
                                '</div>'
                            );
                            var params = self.fillHtml(fileurl, htmlElem, 'prepend');
                            route.title = param.title;
                            tab.data.push(route);
                            layui.chen.render(tab.tabMenuIplId);

                            var currentMenu = $(tab.menu + ' ' + lay);
                            currentMenu.addClass(activeCls);
                            changeView(lay);

                            if ($.ifFunction(callback)) callback(params);
                        })
                    }
                    layui.chen.sidebarFocus(route.href);
                    return false;
                },
                onChange: function() {}
            }
        ...
        self.render = function(fileurl, callback) {
                self.loadHtml(fileurl, function(res) {
                    ...
                    var params = self.findHtml(res.url, htmlElem, 'html');
        ...
        self.renderLayout = function(callback, url) {
            ...
            self.render(url, function(res) {
                ...
                if (conf.viewTabs === true) {
                    self.containerBody.addClass('chen-tabs-body');
                }
                layui.chen.appBody = self.containerBody;
                if ($.isFunction(callback)) callback();
            });
        };
        //加载单页面
        self.renderIndPage = function(fileulr, callback) {
